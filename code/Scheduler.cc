...
void *dispatch_thread_func(void *args) {
    Arg *arg = (Arg*)args ;
    int rank = arg->rank ;
    ...
    for (; ; ) {
        ...
        task_chunkIdx = scheduler->Dispatch(rank) ;
...
void *check_thread_func(void *args) {
    ...
    while (!done) {
        ...
        done = scheduler->Task_complete(rank, task_chunkIdx) ;
    }
...
:size(size), num_reducer(num_reducer), job_name(job_name), locality_config_filename(locality_config_filename), output_dir(output_dir)
{
    ...
    for (int i=0; i<worker_num; i++) {
        ...
        pthread_create(&dispatch_threads[i], NULL, dispatch_thread_func, (void*)&(((Arg*)args)[i])) ;
    }
    check_threads = new pthread_t[worker_num] ;
    for (int i=0; i<worker_num; i++) {
        pthread_create(&check_threads[i], NULL, check_thread_func, (void*)&(((Arg*)args)[i])) ;
    }
}
...
int Scheduler::Dispatch(int target_rank) {
...
int Scheduler::Task_complete(int rank, int task_chunkIdx) {
    int retval ;
    pthread_mutex_lock(&lock) ;
